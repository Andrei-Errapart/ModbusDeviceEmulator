...
namespace ModbusDeviceEmulator
{
    ...
    public class ModbusSlave : IDisposable
    {
        ...
        public const byte FUNCTION_WRITE_SINGLE_REGISTER = 6;
        public const byte FUNCTION_WRITE_MULTIPLE_COILS = 15;
        public const byte FUNCTION_WRITE_MULTIPLE_REGISTERS = 16;

        public const byte MODBUS_RESULT_SUCCESS = 0;
        ...
        private TextboxLogControl _log = null;

        public ModbusSlave(Socket connection, IDictionary<byte, IModbusDataStore> data_store, TextboxLogControl log)
        {
            _log = log;
            ...
            _log.AppendLine("ModbusSlave: New connection from: " + connection.RemoteEndPoint.ToString());
        }
        ...
        protected virtual void Dispose(bool FreeManagedResources)
        {
            if (FreeManagedResources)
            {
                ...
                _log.AppendLine("ModbusSlave: disconnected.");
            }
        }
                ...
                if (max_size>=4)
                {
                    ...
                    switch (function_code)
                    {
                        case FUNCTION_READ_COILS:
                            ...
                            if (max_size >= expected_size)
                            {
                                if (data_store != null && _IsChecksumOK(_read_buffer, processed_sofar, expected_size))
                                {
                                    ...
                                    var dsr = data_store.ReadCoils(wb.Buffer, 3, input_address, input_count);
                                    if (dsr == 0)
                                    {
                                        // GOOD!
                        ...
                        case FUNCTION_READ_DISCRETE_INPUTS:
                            ...
                            if (max_size >= expected_size)
                            {
                                if (data_store!=null && _IsChecksumOK(_read_buffer, processed_sofar, expected_size))
                                {
                                    ...
                                    var reply_size = 5 + byte_count;
                                    var wb = _GetFreeWriteBuffer(reply_size);
                                    var dsr = data_store.ReadDiscreteInputs(wb.Buffer, 3, input_address, input_count);
                                    if (dsr == 0)
                                    {
                                        // GOOD!
                                        // _SendReply(_read_buffer, processed_sofar, expected_size);
                        ...
                        case FUNCTION_READ_INPUT_REGISTERS:
                            goto case FUNCTION_READ_HOLDING_REGISTERS;
                        case FUNCTION_READ_HOLDING_REGISTERS:
                            ...
                            if (max_size >= expected_size)
                            {
                                if (data_store!=null && _IsChecksumOK(_read_buffer, processed_sofar, expected_size))
                                {
                                    ...
                                    var wb = _GetFreeWriteBuffer(reply_size);
                                    var dsr = data_store.ReadHoldingRegisters(wb.Buffer, 3, register_address, register_count);
                                    if (dsr == 0)
                                    {
                                        // GOOD!
                                        // _SendReply(_read_buffer, processed_sofar, expected_size);
                        ...
                        case FUNCTION_WRITE_SINGLE_REGISTER:
                            ...
                            if (max_size >= expected_size)
                            {
                                if (data_store != null && _IsChecksumOK(_read_buffer, processed_sofar, expected_size))
                                {
                                    int register_address = ModbusFormat.UInt16_Of_Bytes(_read_buffer, processed_sofar + 2);
                                    int value = ModbusFormat.UInt16_Of_Bytes(_read_buffer, processed_sofar + 4);
                                    var dsr = data_store.WriteSingleHoldingRegister(register_address, value);
                                    if (dsr == 0)
                                    {
                                        // GOOD!
                                        _SendReply(_read_buffer, processed_sofar, expected_size);
                            ...
                            if (max_size >= 10)
                            {
                                ...
                                if (max_size >= expected_size)
                                {
                                    if (data_store != null && _IsChecksumOK(_read_buffer, processed_sofar, expected_size))
                                    {
                                        ...
                                        var dsr = data_store.WriteMultipleCoils(coil_address, value, coil_count);
                                        if (dsr == 0)
                                        {
                                            // GOOD!
                        ...
                        case FUNCTION_WRITE_MULTIPLE_REGISTERS:
					        if (max_size >= 8)
					        {
                                ...
                                if (max_size >= expected_size)
						        {
							        ...
							        {
								        bool any_good = false;
								        byte dsr = MODBUS_RESULT_SUCCESS;
								        for (int i=0; i<register_count; ++i)
								        {
                                            int contents = ModbusFormat.UInt16_Of_Bytes(_read_buffer, processed_sofar + 7 + 2 * i);
									        byte rr = data_store.WriteSingleHoldingRegister(register_address + i, contents);
								        ...
								        if (any_good)
								        {
                                            _SendSuccessReply(device_address, function_code, register_address, register_count);
                                            /*
									        for (int i=0; i<6; ++i)
									        {
